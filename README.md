...
- `{书名}.exe`会在所在文件夹生成History文件夹用于记录历史记录,在History文件夹下有`{书名}.json`文件记录单本书的阅读记录,通过json文件名区分不同书,**请确保书名唯一避免不同书进度会相互干扰,移动exe/bat/sh/py时请将History文件夹一起移动,否则会丢失进度(会生成新进度)**
